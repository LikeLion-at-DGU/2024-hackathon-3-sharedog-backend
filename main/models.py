...
class Agetest(models.Model):
    ...
    nickname = models.ForeignKey(UserProfile, on_delete=models.CASCADE, default='')
    AGE_CHOICES = [
        ('18개월 미만이에요', '18개월 미만이에요'),
        ('18개월 이상 8세 이하에요', '18개월 이상 8세 이하에요'),
        ('9살 이상이에요', '9살 이상이에요'),
    ]
...
class Weighttest(models.Model):
    ...
    WEIGHT_CHOICES = [
        ('20kg 이하에요', '20kg 이하에요'),
        ('20kg 이상이에요', '20kg 이상이에요'),
    ]

    weight_group = models.CharField(max_length=20, choices=WEIGHT_CHOICES)
...
class Vaccinetest(models.Model):
    ...
    nickname = models.ForeignKey(UserProfile, on_delete=models.CASCADE, default='')
    VACCINE_CHOICES = [
        ('네! 매월 챙기고 있어요', '네! 매월 챙기고 있어요'),
        ('아니요! 매월 챙기지 못했어요', '아니요! 매월 챙기지 못했어요'),
    ]

    is_vaccinated = models.CharField(max_length=30, choices=VACCINE_CHOICES)
...
class Diseasetest(models.Model):
    ...
    nickname = models.ForeignKey(UserProfile, on_delete=models.CASCADE, default='')
    DISEASE_CHOICES = [
        ('네! 앓았던 적이 있어요', '네! 앓았던 적이 있어요'),
        ('아니요! 앓았던 적이 없어요', '아니요! 앓았던 적이 없어요'),
    ]

    has_disease = models.CharField(max_length=30, choices=DISEASE_CHOICES)
